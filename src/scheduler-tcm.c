...
GhtEntry_t ght [MAX_NUMCORES][GHTLEN];
LastReadStat_t lastread [MAX_NUMCORES][LASTREADLEN];
int lastread_head [MAX_NUMCORES], lastread_tail [MAX_NUMCORES];
long long int latestread_time [MAX_NUMCORES];

long long int interval_cnt;
long long int quantum_cnt;
long long int tcm_read_cnt [MAX_NUMCORES];
long long int all_read_cnt[MAX_NUMCORES];
int tcm_priority [MAX_NUMCORES];
int tcm_priority_pos;
int tcm_up;
int gainful_thread [MAX_NUMCORES];
int gainful_thread_ptr [MAX_NUM_CHANNELS];

int gainful_inst_thread [MAX_NUMCORES];
...
void init_gainful_thread()
{
    ...
    //TH_GAIN = 2000 * NUMCORES / NUM_CHANNELS;
    //TH_GAIN = 1000 * NUMCORES / NUM_CHANNELS;
  TH_GAIN = 1000 * NUMCORES;    
  if(NUMCORES == 1){
...
void update_lastread()
{
  ...
  if(interval_cnt > MAX_INTERVAL){
    ...
    int total_read = 0;
    int count_read = 0;
    long long int point[MAX_NUMCORES];
    for (int i = 0; i < NUMCORES; i++){
    ...
    if(NUMCORES == 1){
      ...
      int tmp_pri[MAX_NUMCORES];
      for(int c=NUM_TCM_LATENCY_CLUSTER; c<NUMCORES; c++){
...
{
  request_t * rd_ptr = NULL;
  int gain_count[MAX_NUMCORES];
  int read_count[MAX_NUMCORES];
  ...
  int inst_point [MAX_NUMCORES];
  int inst_pos [MAX_NUMCORES];
  for (int i = 0; i < NUMCORES; i++){
      inst_point[i] = 0;
      inst_pos[i] = 0;
  }
  ...
  for(int i=0; i<NUMCORES; i++){
    ...
    if(t < NUM_TCM_LATENCY_CLUSTER){
      if(i < NUMCORES){
        gain_count[i] += coe_tcm * (NUMCORES * coe_tcm_latencycluster);
      }
    }
    else{
      if(i < NUMCORES){
        gain_count[i] += coe_tcm * (NUMCORES - t -1);
      }
    }
  }
  int tmp_cnt = 0;
...
{
  if(gainful_thread_ptr[channel] < NUMCORES){
...
{
  if(gainful_inst_thread_ptr[channel] < NUMCORES){
...
int precharge(int channel)
{
  ...
  if(NUMCORES == 1 || NUMCORES == 2){
    int close_flag[NUM_RANKS][NUM_BANKS];
    int rank;
    int bank;
    request_t * rd_ptr = NULL;
...
void init_scheduler_vars()
{
  ...
  else if(NUMCORES == 2 && NUM_CHANNELS == 1){
    HI_WM = WQ_CAPACITY - 1;
    LO_WM = HI_WM - 20;
  }
  else if(NUMCORES == 2 && NUM_CHANNELS == 4){
    HI_WM = WQ_CAPACITY - 1;
    LO_WM = HI_WM - 15;
  }
  else if(NUMCORES == 4 && NUM_CHANNELS == 1){
    HI_WM = WQ_CAPACITY - 1;
    LO_WM = HI_WM - 20;
  }
  else if(NUMCORES == 4 && NUM_CHANNELS == 4){
    HI_WM = WQ_CAPACITY - 1;
    LO_WM = HI_WM - 15;
  }
  else if(NUMCORES == 8 && NUM_CHANNELS == 4){
    HI_WM = WQ_CAPACITY - 1;
    LO_WM = HI_WM - 15;
  }
  else if(NUMCORES == 16 && NUM_CHANNELS == 4){
    HI_WM = WQ_CAPACITY - 5;
    LO_WM = HI_WM - 20;
  }
  else {
